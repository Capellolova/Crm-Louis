...
def page_dashboard():
    ...
    with left:
        ...
        if affairs.empty:
            ...
            pipe = affairs.groupby('status', dropna=False).agg(nb=('id', 'count'), total=('total_estimated', 'sum')).reset_index()
        ...
        if not affairs.empty:
            ...
            st.dataframe(top[['client_name', 'category', 'status', 'total_estimated', 'next_action_date']], use_container_width=True, hide_index=True)
    with right:
        ...
        if not affairs.empty:
            ...
            if sleep.empty:
                ...
                st.dataframe(sleep[['client_name', 'status', 'days_without_activity', 'next_action_date']], use_container_width=True, hide_index=True)
...
def page_affairs():
    ...
    st.dataframe(filtered[['client_name', 'priority', 'opportunity_type', 'category', 'status', 'monthly_rent', 'next_action_date', 'competitor', 'blockage']], use_container_width=True, hide_index=True)
...
def page_today():
    ...
    st.dataframe(ao_df[['client_name', 'category', 'ao_deadline', 'status']], use_container_width=True, hide_index=True)
    st.subheader('Affaires chaudes inactives')
    st.dataframe(hot_df[['client_name', 'status', 'days_without_activity', 'monthly_rent']], use_container_width=True, hide_index=True)


def page_stats():
    st.title('Stats & pilotage')
    affairs = get_affairs()
    if affairs.empty:
    ...
    by_cat = affairs.groupby('category', dropna=False).agg(nb=('id', 'count'), total=('total_estimated', 'sum')).reset_index()
    st.dataframe(by_cat, use_container_width=True, hide_index=True)
    st.subheader('Concurrence')
    by_comp = affairs.fillna({'competitor': 'Non renseigné'}).groupby('competitor').agg(nb=('id', 'count')).reset_index().sort_values('nb', ascending=False)
    st.dataframe(by_comp, use_container_width=True, hide_index=True)
    st.subheader('Blocages')
    by_block = affairs.fillna({'blockage': 'Non renseigné'}).groupby('blockage').agg(nb=('id', 'count')).reset_index().sort_values('nb', ascending=False)
...
def render_sidebar():
    ...
    page = st.sidebar.radio('Aller vers', ['🏠 Accueil', '📂 Affaires', '🏢 Clients', '➕ Ajouter', '📊 Stats', '🚨 Aujourd’hui'])
    st.sidebar.markdown('---')
